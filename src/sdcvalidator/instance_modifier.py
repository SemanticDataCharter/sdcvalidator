...
class InstanceModifier:
    ...
    def insert_exceptional_value(self,
        ...
        if invalid_elem is None:
            logger.warning(
                "Could not find element at xpath '%s' for ExceptionalValue insertion", xpath
            )
            return False
        ...
        if parent_elem is None:
            logger.warning(
                "Could not find parent element for '%s' at xpath '%s'",
                invalid_elem.tag, xpath
            )
            return False
    ...
    def _register_namespace(self):
        ...
        try:
            ...
            logger.debug("Namespace registration skipped (may already be registered): %s", e)
    ...
    def _find_element_by_xpath(self, root: ET.Element, xpath: str) -> Optional[ET.Element]:
        ...
        try:
            ...
            logger.debug("XPath lookup failed for '%s': %s; trying manual parse", xpath, e)
    ...
    def _find_insertion_position(self, parent: ET.Element) -> int:
        ...
        for i, child in enumerate(parent):
            ...
            if local_name in EXCEPTIONAL_VALUE_CODES:
                continue
